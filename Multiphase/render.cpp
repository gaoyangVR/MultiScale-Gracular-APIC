...
void cspray::render()
{
	...
	//if (simmode == SIMULATION_BUBBLE || simmode == SIMULATION_SMOKE)
	//{
	...
	//	}
	//}
	if (simmode == SIMULATION_WATER || simmode == SIMULATION_SOLIDCOUPLING || simmode == SIMULATION_BUBBLE || simmode == SIMULATION_HEATONLY || simmode == SIMULATION_APICBASIC)
	{
	...
	else if (simmode == SIMULATION_SMOKE || m_bSmoke)
	{
		if (rendermode == RENDER_PARTICLE || rendermode == RENDER_ALL)
			smokeRayCasting();
