...
__global__ void setgridcolor_k(float* color, ECOLORMODE mode, farray p, farray ux, farray uy, farray uz, farray div, farray phi, charray mark, farray dense, farray ls, farray tp, float sigma, float temperatureMax, float temperatureMin);
...
__global__ void mapvelp2g_k_Fluid_APIC(float3 *mParcpx, float3 *mParcpy, float3 *mParcpz, float3 *pos, float3 *vel, float *mass, char *parflag, int pnum, farray ux, farray uy, farray uz, uint* gridstart, uint *gridend);
...
__global__ void updatesprayvel_k(farray sprayux, farray sprayuy, farray sprayuz, farray dense, farray waterux, farray wateruy, farray wateruz, charray mark, float densegravityMul, float3 wind);
__global__ void markwaterparticle_par(float3* ppos, float3* pvel, float* pmass, char* pflag, bool* pparstaymark, int pnum, farray ux, farray uy, farray uz, charray mark, uint *transmark, float genSprayRelVelThres, float genSprayVelThres);
...
__global__ void genSprayParticles(int activeparticle, int splitnum, int parnum, uint *compactParticles, float3* pos, float3 *vel, char* parflag, float *parmass, float3 *parrandom, int *parranindex, farray ux, farray uy, farray uz, float *randfloat, int randfloatcnt);
__global__ void Markcelltrans(farray ux, farray uy, farray uz, farray spraydense, uint* transcellid, uint* denseminusmark, float genSandVelThres, float notgenSandvelThres);
__global__ void genSandParticles(int activecell, int splitnum, int parnum, uint* compactcells, float3* pos, float3 *vel, char* parflag, float *parmass, bool *parstaymark, farray ux, farray uy, farray uz, float *randfloat, int randfloatcnt, int frame, farray spraydense, uint* denseminusmark);
__global__ void genSprayParticles_radius(int activeparticle, int splitnum, int parnum, uint *compactParticles,	float3* pos, float3 *vel, char* parflag, float *parmass, farray ux, farray uy, farray uz, float *randfloat, int randfloatcnt);
__global__ void updatespraydensity_k(charray mark, farray waterux, farray wateruy, farray wateruz, farray dens, farray sprayux, farray sprayuy, farray sprayuz, farray div);
__global__ void updateSprayDensityFromParticle_k(farray ux, farray uy, farray uz, farray spraydense, uint *gridstart, uint *gridend, charray mark, farray massadd, float3* ppos, float3* pvel, float *pwsum, uint *transmark, float* transmass);
__global__ void sumweight(float3* ppos, uint* transmark, float *pwsum, int pnum, charray mark);
__global__ void Transparticle_Spray2Sand(float3* ppos, float3* pvel, float* pmass, char* pflag, int pnum, farray ux, farray uy, farray uz, charray mark, uint *transmark, float genSprayRelVelThres, float genSprayVelThres, farray spraydense);
__global__ void MinusSprayDensityFromParticle(farray ux, farray uy, farray uz, farray spraydense, uint *gridstart, uint *gridend, charray mark, farray massadd, float3* ppos, float3* pvel, float *pwsum, uint *transmark, float* transmass, uint* minusmark);
