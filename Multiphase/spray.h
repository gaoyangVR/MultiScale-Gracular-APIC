...
//const int NX=96,NY=96,NZ=64;  
//const int NX=128,NY=128,NZ=128;	//melting 
//const int NX = 96, NY = 96, NZ = 96;  //boiling high resolution
//const int NX=128,NY=64,NZ=128;
//const int NX = 96, NY = 96, NZ = 96;//freezing meltingpour
//const int NX = 128, NY = 128, NZ = 64;
//const int NX=48,NY=32,NZ=64;  //interaction
const int NX=64,NY=64,NZ=64; //boiling / melting
//const int NX=24,NY=24,NZ=96;
//const int NX = 64, NY = 64, NZ = 128;//SANDSTORM
...
public:
	...
	//void smokeinit();
	void calculateNum(int *activeParticleNum);
	void updatedensity();
	void generatespray();
	void generatesand();
	void APIC_basicsim();
	void APIC_watersolidsim();
	void smokeadvection();
	void smokesim();
	void smokemarkgrid();
	...
	uint *parTransMark, *parTransMarkScan, *parmarkcompact;		//标记是否要转为spray particle
	uint *cellTransMark, *cellTransMarkScan, *cellmarkcompact;
	uint *denseTransMark, *denseMinusMark;
	float *denseWeightSum, *denseTransMass;
	farray massadd;
	...
	int cntAirParMax, cntLiquidMax, cntSolidMax, cntSandMax, cntSmokeMax;

	FlipConstant hparam;
	static const int threadnum = 512;
	int gsblocknum, gvblocknum, pblocknum;		//CUDA block个数：标量场、向量场、粒子

	//data for eulerian field of solid.
	f3array solidvel;
	...
	charray mmark, mark_terrain, sandmark;
	//farray waterweight, watergpos;
	matarray strainrateD, stresstensor;
	f3array yieldvel;
	int cellcount;

	//for FLIP particle ( liquid particle )
	float3 *mParPos, *mParVel, *tmpParPos, *tmpParVelFLIP;
	float3 *mParRandomPos;
	int *mParRanIndex, *tempParRanIndex;
	...
	/////xu cluster number defination
	int* clu_num;
	//APIC
	//float3 *Par_am;
	matrix3x3 *mParMatB;
	float3 *mParcpx, *mParcpy, *mParcpz;
	...
	float genSprayRelVelThres, genSprayVelThres, genSandVelThres, notgenSandvelThres;
	float fWater2Spray;			//从water直接转成spray的scale，一般为1
	...
	bool solver_sand(matarray D, matarray sigma, farray press, charray sandmark, f3array yieldvel, farray ux, farray uy, farray uz, int num);
	void connect_dfs(int idx, bool* visited, int* connect_id, int connect_count, farray ux, farray uy, farray uz, charray sandmark, float3* v_bar, float3* w_bar, float3* x_bar, float* rigid_mass, int &rigidnum);
	void advect_bubble();
	...
	bool boutputpovray, bOutputColoredParticle,boutputobj, boutputfile;
	...
	void outputDensity(int frame);
	void outputParticle(int frame, float3 *dpos, float *dmass, char *dflag, int pnum);
	void outputParticleFile(int frame, float3 *dpos, float *dmass, char *dflag, float3 *drandom, int *dranindex, int pnum);
	void outputBPhysFile(int frame, float3 *dpos, float *dmass, char *dflag, int pnum);
	void outputSmokeFile(int frame, float3 *dpos, float *dmass, char *dflag, int pnum);
	void outputspherePOVRay(int frame, float3 pos, float radius);
	...
	int nInitSolPoint, Pointnum, nRealSolpoint, rigidparnum;
